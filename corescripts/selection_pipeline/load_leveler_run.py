...
class LoadLevelerRun(object):
    ...
    def create_load_leveler_script(self,options,config):
        if(options.phased_vcf): 
           (dependecies,haps) = self.ancestral_annotation_vcf(options,config,dependencies)
           (dependencies,haps) = self.run_multi_coreihh(options,config,haps,dependencies)
        else:
            (dependencies,ped,map) = self.run_vcf_tools(options,config,dependencies)
            (dependencies,ped,map) = self.run_plink(options,config,ped,map,dependencies)
    ...
    def run_vcf_to_plink(self,options,config):
        ...
        memory_required="4"
        ulimit=memory_required=str(int(memory_required) * 1024 * 1024)
        step_name=prefix+str(datetime.now()).strip()
        self.load_leveler_script.write(self.load_leveler_step.format(step_name,"11:59:00",memory_required))
        self.load_leveler_script.write(self.load_leveler_queue)
        logger.debug("Finished preparing vcf_to_plink for running on pan")
        return(step_name,prefix + '.ped', prefix+'.map',) 
