...
To download the example dataset enter the
following command:
...
the folder in which the example data were extracted, and change the
...
\end{minted}
\noindent
{\bf\small\color{blue} [*** JAMES - I added backslashes to the code
  above in case people try to paste directly in the command line -
  good or stupid?  ***]} 
...
plot(weirFST[,6]~ weirFST[,2], pch=16, cex=.4, type="p",xlab='',ylab=expression("F[ST]")) 
abline(mean(weirFST[,6]) + 3 * sd (weirFST[,6])col='red')
\end{minted}

\begin{figure}
\centering
\includegraphics{pictures/yriCEUYRI.png}
...
plot(hapmapFST[,6]~ hapmapFST[,2], pch=16, cex=.4, type="p",xlab='',ylab=expression("F[ST]")) 
abline(mean(hapmapFST[,6]) + 3 * sd (hapmapFST[,6])col='red')
\end{minted}
\begin{figure}
\centering
...
#Plot Fay and Wu's H
plot(YRIFay[,15] ~ YRIFay[,1],xlim=c(136545410-1e6,136594750+1e6),pch='.',cex=2,ylim=c(-100,0)
xlab='',ylab="H Statistic")
...
#plot IHS pvalues
plot(CEUihs[,4] ~ CEUihs[,2],xlim=c(1.364e8,1.368e8),pch='.',cex=.5,ylab=expression("-" * log[10] * "[" ~ "1-2|" * Phi[scriptstyle(italic(iHS))] * "-0.5|" ~ "]"),xlab="Chromosome Position BP") 
...
#plot IHS pvalues
plot(YRIihs[,4] ~ YRIihs[,2],xlim=c(1.364e8,1.368e8),pch='.',cex=.5,ylab=expression("-" * log[10] * "[" ~ "1-2|" * Phi[scriptstyle(italic(iHS))] * "-0.5|" ~ "]"),xlab="Chromosome Position BP") 
rect(136545410,-10,136594750,10,border="Blue") 
abline(h=2,col="red")
\end{minted}
...
plot(tajimaD[,4] ~ tajimaD[,2],pch='.',cex=2)
rect(136545410,-10,136594750,10,border="Blue") 
\end{minted}
...
The rehh package source included with the pipeline has been altered to
match the output filters used in Voight's paper. If the EHH > 0.05
reaches the end of a chromosome or the start of a gap >
big\_gap\_threshold , then no value is returded for the core snp. The
small\_gap\_threshold specifies the gap distance to reduce the
distance spanned by the gap by a multiplicative factorpecified by
