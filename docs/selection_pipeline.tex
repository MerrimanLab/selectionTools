...
\noindent
{\bf\small\color{blue} [*** JAMES - WHATS UP WITH THE UGLY GREEN LINKS FOR
  CITATIONS?  ;) ***]}
...
system. 20GB of RAM should be sufficient for all computation steps
...
\item Perl5
\item R \( >= 3.0.0 \) (with a personal library) {\bf\small\color{blue} [*** JAMES
    - WHAT IS A ``PERSONAL LIBRARY''? ***]}
\item Build-essential tools {\bf\small\color{blue} [*** JAMES
    - SPECIFICALLY...? ***]}
\end{itemize}
...
{\bf\small\color{blue}  [*** JAMES - NEED TO TELL THEM HOW TO GET THE FILES ***]}

\noindent
To install a standalone version of the pipeline (which will require
manual adjustment of the configuration file), run the following command:\\
...
\noindent
{\bf\small\color{blue} [*** JAMES - NEED TO EXPLAIN THE DIFFERENCE BETWEEN MANUAL
  AND AUTOMATIC INSTALLATION ***]}

\noindent
The remainder of this section is dedicated to the automatic
installation. To perform an automatic installation of the selection
analysis pipeline, run the command.\\
\begin{minted}[bgcolor=bg,frame=lines]{bash}
./install.sh
\end{minted}
...
setting some options in the config file. The question mark character
"?" in the config is substituted by the chromosome number: this is
used for reference files that are split on chromosomes.\\

\noindent
{\bf\small\color{blue} [*** JAMES - IS THAT ENOUGH DETAIL FOR USING
...
sure to extract contents of the archive after download. The default
directory to store the ancestral reference files is
referencefiles/ancestral\_ref/.
...
In humans, the lactase is encoded by the LCT gene, which is located on
...
those of European ancestory.  
{\bf\small\color{blue} [*** JAMES - CITATION? ***]}.  \\

\begin{minted}[bgcolor=bg]{bash}
wget http://tutorial_file_location.com 
\end{minted}
\noindent
{\bf\small\color{blue} [*** JAMES - I RECKON THATS NOT THE REAL
  LOCATION.  :)  ***]}.  \\
\noindent
...
step. Population files are line seperated files the first line
contains the population name every successive line contains and
